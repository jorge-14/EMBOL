...
import { GestionDotacion, GestionDetalle, MesDotacion, EmpleadoDotacion, MovimientoPlanificado } from '../models/dotacion.model';
...
export class DotacionService {
  ...
  private gestionesMock: GestionDotacion[] = [
  ...
  private empleadosMock: EmpleadoDotacion[] = [
    {
  ...
  private movimientosMock: MovimientoPlanificado[] = [
    {
  ...
  getGestiones(): Observable<GestionDotacion[]> {
    return of(this.gestionesMock);
  }

  getGestionById(id: number): Observable<GestionDetalle | undefined> {
    // ... existiendo
    const gestion = this.gestionesMock.find(g => g.id === id);
    ...
    const meses: MesDotacion[] = mesesNombres.map((nombre, index) => {
      ...
      return {
        nombre,
        empleados: isCargado ? 310 + (index * 2) : 0,
        disponible: gestion.anio <= 2026 || (gestion.anio === 2027 && index === -1), // Simulación de disponibilidad
        cargado: isCargado
      };
    });


    if (gestion.anio === 2027) {
        meses.forEach(m => {
            m.disponible = false;
            m.cargado = false;
        });
    ...
    const detalle: GestionDetalle = {
      id: gestion.id,
      anio: gestion.anio,
      sucursal: gestion.sucursal || 'N/A',
      ciudad: gestion.ciudad || 'N/A',
  ...
  getEmpleadosByGestion(gestionId: number, mes?: string): Observable<EmpleadoDotacion[]> {
    return of(this.empleadosMock);
  }

  getMovimientosByGestion(gestionId: number): Observable<MovimientoPlanificado[]> {
    return of(this.movimientosMock);
  }
}
